---
title: Requirements
description: Normative product contract for Lasso core.
---

## LR-1: Agent runtime is the execution kernel

Codex is the design baseline. Claude Code and Grok Build are followers. Lasso
MUST NOT mirror task, goal, plan, progress, review, memory, or session state.
...
surface, the residual workspace gap, and a deletion trigger.

## LR-3: Customization uses agent extension surfaces

Prefer `AGENTS.md`, deterministic gates, plugins, skills, and MCP over a
parallel agent framework. One canonical skill tree; runtime-specific discovery
adapters only.
...
gates plus `verify-change` when spanning projects or closing work items.
